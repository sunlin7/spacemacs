...
;;;###autoload
(defcustom spacemacs-quickstart-root
  spacemacs-cache-directory
  "Location of the file used to speed up activation of spacemacss at startup."
  :type 'file
  :group 'spacemacs
...
;;;###autoload
(defun spacemacs-quickstart-refresh ()
  "(Re)Generate the `spacemacs-quickstart-file'."
  (interactive)
  (let (layers-files-alist
        ...
        (print-length nil)
        (print-level nil))
    (dolist (x configuration-layer--used-layers)
      (when-let* ((layer-obj (configuration-layer/get-layer x))
                  (layer-dir (oref layer-obj :dir)))
        (dolist (filepath (file-expand-wildcards (concat layer-dir "*.el") t))
          (when-let ((base-name (file-name-base filepath)))
            (if-let ((field (assoc base-name layers-files-alist)))
                (push filepath (cdr field))
              (push (cons base-name (list filepath)) layers-files-alist))))))

    (dolist (x layers-files-alist)
      (let ((quickstart-file (format "%s/%s.el" spacemacs-quickstart-root (car x)))
            (pfile))
        (with-temp-file quickstart-file
          (emacs-lisp-mode)                 ;For `syntax-ppss'.
          (insert ";;; Quickstart file to activate all spacemacs layers at startup  -*- lexical-binding:nil -*-\n")
          (insert ";; ¡¡ This file is autogenerated by `spacemacs-quickstart-refresh', DO NOT EDIT !!\n\n")
          (dolist (file (cdr x))
            (setq pfile (prin1-to-string file))
...
\(load-file-name " pfile "))\n")
            ...
            (unless (bolp) (insert "\n"))
            (insert ")\n"))
          ;; (pp `(setq spacemacs-activated-list
          ;;            (delete-dups
          ;;             (append ',(mapcar #'spacemacs-desc-name spacemacs--quickstart-pkgs)
          ;;                     spacemacs-activated-list)))
          ;;     (current-buffer))
          ;; Use `\s' instead of a space character, so this code chunk is not
          ;; mistaken for an actual file-local section of spacemacs.el.
          (insert "
;; Local\sVariables:
...
"
        ...
        ;; (byte-compile-file quickstart-file)
        ))
    ))

;;;###autoload
(defun spacemacs-quick-load (name)
  (load (expand-file-name name spacemacs-quickstart-root) t))

(defvar indexed-layers-file-base "/tmp/indexed-layers")
;;;##autoload
(defun spacemacs-quick-var-refresh ()
  "(Re)Generate the layer variables"
  (interactive)
  (let ((fpath (concat indexed-layers-file-base ".el")))
    (with-temp-file fpath
      (princ "(setq configuration-layer--indexed-layers\n")
      (prin1 configuration-layer--indexed-layers)
      (princ ")\n")
      (princ "(setq configuration-layer--used-layers\n'")
      (prin1 configuration-layer--used-layers)
      (princ ")\n")
      (princ "(setq configuration-layer--indexed-packages\n")
      (prin1 configuration-layer--indexed-packages)
      (princ ")\n")
      (princ "(setq configuration-layer--used-packages\n'")
      (prin1 configuration-layer--used-packages)
      (princ ")\n"))
    (byte-compile-file fpath)))
...
`spacemacs-sync-packages' to nil."
  ...
  ;; usage and ownership
  (load indexed-layers-file-base)
  (let ((configuration-layer--load-packages-files t))
    (spacemacs-quick-load "layers")
    (spacemacs-quick-load "packages"))
  ...
  ;;                                         '("funcs"))
  (spacemacs-quick-load "funcs")
  (spacemacs-quick-load "config")
  ...
  ;; (configuration-layer//load-layers-files configuration-layer--used-layers
  ;;                                         '("keybindings"))
  (spacemacs-quick-load "keybindings")
      ...
      (let* ((dir (file-name-as-directory dir))
             ...
             (shadow
              (if (and (listp layer-specs)
                ...
                'unspecified))
             (packages (let ((symbol (intern (format "%S-packages" layer-name))))
                         (if (boundp symbol)
                             (symbol-value symbol)
                           (when-let (((null packages))
...
USEDP non-nil means that PKG is a used layer."
  (when-let ((_ spacemacs-quickstart-root)
             (layer-name (oref layer :name)))
    ...
    (if layer
        (let ((obj (configuration-layer/make-layer
          ...
          (when (and (not skip-layer-deps)
            ...
            (unless spacemacs-quickstart-root
...
(defun configuration-layer/declare-layer-dependencies (layer-names)
  ...
  (dolist (x layer-names)
    (unless (member x configuration-layer--layers-dependencies)
      ...
      (unless (or configuration-layer--declared-layers-usedp
                  spacemacs-quickstart-root)
...
(defun configuration-layer//load-layer-files (layer-name files)
  ...
  (when-let ((obj (configuration-layer/get-layer layer-name)))
    (dolist (file files)
      (configuration-layer/load-file (concat (oref obj :dir) file) t))))
