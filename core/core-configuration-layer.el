...
;;;###autoload
(defcustom spacemacs-quickstart-file
  (expand-file-name "spacemacs-quickload.el" spacemacs-cache-directory)
...
(defun spacemacs-quickstart-get-loader (layername filename)
  (intern (format "spacemacs-quickstart--%s-%s" layername filename)))
...
(defun spacemacs-quickstart-refresh ()
  ...
  (let (;; Make sure we can load this file without load-source-file-function.
    ...
    (with-temp-file spacemacs-quickstart-file
      ...
      (dolist (x configuration-layer--used-layers)
        (when-let* ((layer-obj (configuration-layer/get-layer x))
                    ...
                    (layer-name (oref layer-obj :name)))
          (dolist (file (file-expand-wildcards (concat layer-dir "*.el") t))
            (let ((pfile (prin1-to-string file))
                  (bname (file-name-base file)))
              (insert (format "(defun %s ()\n" (spacemacs-quickstart-get-loader
                                                layer-name bname)))
      ...
      ;; (pp `(defvar spacemacs-activated-list) (current-buffer))
...
"
    ...
    ;; (byte-compile-file spacemacs-quickstart-file)
    ))
...
`spacemacs-sync-packages' to nil."
  ...
  ;; usage and ownership
  (load "~/.emacs.d/.cache/spacemacs-quickload" t)
    ...
    (if (or (null dir)
      ...
      (let* ((dir (file-name-as-directory dir))
             ...
             (packages (when (and (null packages)
                         ...
                         (funcall (spacemacs-quickstart-get-loader layer-name "packages"))
...
(defun configuration-layer//load-layer-files (layer-name files)
  ...
  (when-let ((obj (configuration-layer/get-layer layer-name)))
    (dolist (file files)
      (if-let* ((loader (spacemacs-quickstart-get-loader
                         layer-name file))
                ((fboundp loader)))
          (funcall loader)
        ;; (message "load by file: %s, function not available: %s" file loader)
